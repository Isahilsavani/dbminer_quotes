...
class _AllQuotesState extends State<AllQuotes> {
                              ...
                              child: Column(
                                children: [
                                  Container(
                                    height: 380,
                                    decoration: BoxDecoration(
                                        image: DecorationImage(image: NetworkImage("${AllQuotes?[i].image}"), fit: BoxFit.cover),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      InkWell(
                                          onTap: () {
                                            setState(() {
                                              AllQuotes?[i].image = AllQuotes[Counter].image;
                                              if (Counter < 40) {
                                                Counter++;
                                              }
                                            });
                                      ...
                                      InkWell(
                                        onTap: () async {
                                          ...
                                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Dowoload..")));
                                        },
                                        child: Icon(
                                      ...
                                      InkWell(
                                        onTap: () {
                                          ...
                                          LikeDbHelper.likeDbHelper
                                              .insertlikeData(quotes: AllQuotes![i].quote.toString(), image: AllQuotes[i].image.toString());
                                        },
                                        child: (starList[i])
                                            ? const Icon(
                                                Icons.star,
  ...
  Widget SSContainer({required String image, required String quote}) {
    return Container(
      ...
      width: double.infinity,
      decoration: BoxDecoration(
          boxShadow: const [BoxShadow(blurRadius: 10, color: Colors.grey)], borderRadius: BorderRadius.circular(20), color: Colors.white),
      child: Column(
        children: [
          Container(
            height: 440,
            decoration: BoxDecoration(
                image: DecorationImage(image: NetworkImage(image), fit: BoxFit.cover), borderRadius: BorderRadius.circular(0), color: Colors.black38),
            child: Container(
              height: 440,
              color: Colors.black.withOpacity(0.45),
              child: Padding(
